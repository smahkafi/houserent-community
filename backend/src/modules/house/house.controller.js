...
import {
  ...
  reviewRestoreSchema,
  permanentDeleteHouseSchema,
} from "./house.validation.js";

const createHouse = async (req, res) => {
  try {
    const data = createHouseSchema.parse(req.body);
    const result = await houseService.createHouse(data, req.user);
...
const getAllApprovedHouses = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: "Approved houses fetched successfully",
      data,
    });
  } catch (e) {
  ...
  try {
    ...
    if (!data) {
      return res.status(404).json({
        success: false,
        message: "House not found or not approved",
      });
    }
...
const getMyHouses = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: "My houses fetched successfully",
      data,
    });
  } catch (e) {
    return res.status(500).json({
...
const softDeleteHouse = async (req, res) => {
  try {
    ...
    const result = await houseService.softDeleteHouse(
      req.params.id,
      data.deleteReason,
      req.user
    );
...
const requestRestoreHouse = async (req, res) => {
  try {
    ...
    const result = await houseService.requestRestoreHouse(
      req.params.id,
      data.restoreReason,
      req.user
    );
...
const getDeletedHouses = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: "Deleted houses fetched successfully",
      data,
    });
  } catch (e) {
    return res.status(500).json({
...
const getRestoreRequests = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: "Restore requests fetched successfully",
      data,
    });
  } catch (e) {
    return res.status(500).json({
...
const reviewRestoreRequest = async (req, res) => {
  try {
    ...
    const result = await houseService.reviewRestoreRequest(
      req.params.id,
      data.action,
      data.note,
      req.user
    ...
    return res.status(200).json({
      ...
      message:
        data.action === "APPROVE"
          ? "House restore approved successfully"
          : "House restore rejected successfully",
      data: result,
    });
  } catch (e) {
    return res.status(400).json({
...
const permanentDeleteHouse = async (req, res) => {
  try {
    ...
    const result = await houseService.permanentDeleteHouse(
      req.params.id,
      data.reason,
      req.user
    );
...
const getHouseAdminSummary = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: "House admin summary fetched successfully",
      data,
    });
  } catch (e) {
    return res.status(500).json({
