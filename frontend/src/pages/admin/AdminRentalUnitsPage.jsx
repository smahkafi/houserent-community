...
function AdminRentalUnitsPage() {
  ...
  const fetchUnits = async () => {
    try {
      ...
      const res = await axios.get(`${API_BASE_URL}/rental-units/pending`, {
  ...
  const handleApprove = async (id) => {
    ...
    try {
      const token = getToken();
      await axios.patch(
        `${API_BASE_URL}/rental-units/${id}/approve`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      await fetchUnits();
  ...
  const handleReject = async () => {
    ...
    try {
      const token = getToken();
      await axios.patch(
        `${API_BASE_URL}/rental-units/${rejectModal}/reject`,
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-950 px-4 py-10 text-white">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mb-8">
          ...
          <h1 className="mt-1 text-3xl font-bold">Manage Rental Units</h1>
          <p className="mt-1 text-sm text-slate-400">Pending rental units waiting for approval.</p>
        </div>
        ...
        {!loading && !error && units.length === 0 && (
          <div className="rounded-3xl border border-white/10 bg-white/5 p-10 text-center backdrop-blur-md">
            ...
            <p className="mt-3 font-semibold text-slate-200">No pending rental units</p>
            <p className="mt-1 text-sm text-slate-400">All rental units have been reviewed.</p>
          </div>
        )}
        ...
        {!loading && !error && units.length > 0 && (
          <div className="space-y-4">
            {units.map((unit) => (
              <div key={unit.id} className="rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur-md">
                <div className="mb-3">
                  <h2 className="text-lg font-semibold text-white">{unit.title}</h2>
                  <p className="text-sm text-slate-400">
                    {unit.house?.title} — {unit.house?.address}, {unit.house?.area}
                  </p>
                  <p className="text-sm text-slate-400">
                    👤 Landlord: {unit.house?.landlord?.fullName} — {unit.house?.landlord?.phone}
                  </p>
                </div>
                <div className="mb-4 grid grid-cols-2 gap-2 text-sm text-slate-300">
                  ...
                  {unit.availableFrom && <p>📅 From {new Date(unit.availableFrom).toLocaleDateString()}</p>}
                </div>
                {unit.description && (
                  <p className="mb-4 text-sm text-slate-400">{unit.description}</p>
                )}
                <div className="flex gap-3">
                  ...
                  >
                    {actionLoading === unit.id ? "Processing..." : "Approve"}
                  </button>
                  <button
                    onClick={() => { setRejectModal(unit.id); setRejectReason(""); }}
                    disabled={actionLoading === unit.id}
        ...
        {rejectModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
            <div className="w-full max-w-md rounded-3xl border border-white/10 bg-slate-900 p-6 text-white">
              <h3 className="mb-4 text-lg font-semibold">Reject Rental Unit</h3>
              <textarea
                value={rejectReason}
...
export default AdminRentalUnitsPage;